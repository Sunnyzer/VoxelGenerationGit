...
public class OldChunk : MonoBehaviour
{
    ...
    [SerializeField] List<OldChunk> chunkNeighbor = new List<OldChunk>();
    ...
    void GenerateBlocksChunk()
    {
        for (int x = 0; x < chunkSize; x++)
        {
            ...
            {
                float noiseValue = Mathf.PerlinNoise((OldChunkManager.x + transform.position.x + x) * noiseScale, (OldChunkManager.y + transform.position.z + z) * noiseScale);
    ...
    Vector3Int[] IsBlockFill(Vector3Int _pos)
    {
        ...
        foreach (Vector3Int _dir in direction)
        {
            ...
            else if (_dir != Vector3.up && _dir != Vector3.down)
            {
                ...
                OldChunk _chunkNeighbor = OldChunkManager.Instance.GetChunk((int)_posCube.x, (int)_posCube.z);
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        {
            OldChunk _chunk = chunkNeighbor[i];
